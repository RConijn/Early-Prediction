...
cutpastejump <- keylog_trim %>%
  ...
  group_by(pid) %>%
  summarise(
    cut_paste_jump_m = mean((time_cut_paste_jump), na.rm = TRUE),
    cut_paste_jump_sd = sd((time_cut_paste_jump), na.rm = TRUE)
  ) %>%
# add NA for pid with no cut/paste/jump events.
  rbind(c(7,NA,NA))
...
correct <- keylog_trim %>%
  ...
  summarise(n_backspaces = n(),
            ...
            time_backspaces = ifelse(time_backspaces >
                                     ...
                                     time_backspaces)) %>%
  group_by(pid) %>%
  summarise(single_backspacetime_m = mean((time_backspaces[n_backspaces == 1]),
                                          na.rm = TRUE),
            single_backspacetime_sd = sd((time_backspaces[n_backspaces == 1]),
                                          na.rm = TRUE),
            multi_backspacetime_m = mean((time_backspaces[n_backspaces > 1]),
                                          na.rm = TRUE),
            multi_backspacetime_sd = sd((time_backspaces[n_backspaces > 1]),
                                         na.rm = TRUE)
            )
...
keyfeatures <- keylog_trim  %>%
  ...
  summarise(
    ...
    iki_median = median((iki_trim),  na.rm = TRUE),
    iki_largest = max(iki,  na.rm = TRUE),
    #time-based features
    iki_m = mean((iki_trim), na.rm = TRUE),
    iki_sd = sd((iki_trim), na.rm = TRUE),
    iki_inword_m = mean((iki_inword_trim), na.rm = TRUE),
    iki_inword_sd = sd((iki_inword_trim), na.rm = TRUE),
    iki_betwword_m = mean((iki_betwword_trim), na.rm = TRUE),
    iki_betwword_sd = sd((iki_betwword_trim), na.rm = TRUE),
    betweenwords_m = mean((betweenwords_trim), na.rm = TRUE),
    betweenwords_sd = sd((betweenwords_trim), na.rm = TRUE),
    betweensentence_m = mean((betweensentence_trim), na.rm = TRUE),
    betweensentence_sd= sd((betweensentence_trim), na.rm = TRUE),
    # ratio-based values
  ...
  left_join(select(grades, pid, c.total), by = "pid") %>%
  # remove copy-paster (pid ==82), typed in other word doc (pid == 27,55) and
  # person who did not do the task (pid == 91)
  filter(n_keystrokes >= 500,
         n_words >= 50)

# set missing data to avg
keyfeatures <- do.call(data.frame,lapply(keyfeatures,
                                         function(x) replace(x, !is.finite(x),
                                                             mean(x, na.rm = TRUE))))

#write all
write.csv(keyfeatures,"data/keystrokes_feat_30mins.csv", row.names = FALSE)
