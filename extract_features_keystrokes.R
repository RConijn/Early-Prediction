...
group_data_keys <- function(keystrokeframe){

  timeframe_start <- as.numeric(substr(keystrokeframe, 1,2))
  timeframe_end <- as.numeric(substr(keystrokeframe, 3,4))


uptotime <- keylog_trim %>%
    group_by(pid) %>%
    summarise(total_keystrokes = max(keystroke_no, na.rm = TRUE),
              endkeystroke = round(timeframe_end/30 * total_keystrokes),
              timeend = endTime[which(keystroke_no == endkeystroke)],
              startkeystroke = round(timeframe_start/30 * total_keystrokes),
              startkeystroke = ifelse(startkeystroke == 0, 1, startkeystroke),
              timestart = startTime[which(keystroke_no == startkeystroke)]
              ) %>%
  select(pid, timeend, timestart)

keylog_trim <- keylog_trim %>%
  left_join(uptotime, by = "pid") %>%
  filter(endTime <= timeend,
         startTime > timestart)
...
keyfeatures <- do.call(data.frame,lapply(keyfeatures, function(x) replace(x, !is.finite(x),0)))
  ...
  write.csv(keyfeatures,
            paste0("data/keystroke/keystrokes_from_", timeframe_start, "_to_",
                   timeframe_end, "_keystrokes.csv"),
            row.names = FALSE)
}
...
######## ---------- WRITE FEATURE DATA -----------------------------------######
# write cleaned data upto every 1/6th keystroke
lapply(c("0005", "0010", "0015", "0020","0025", "0030",
         "0510", "1015","1520", "2025", "2530"), group_data_keys)
